...
while True:
    ...
    pxnew = np.array(random2.choices(population=px, weights=lp, k=Np))\
            + np.random.randint(-15, 15, Np)
    pynew = np.array(random2.choices(population=py, weights=lp, k=Np))\
            + np.random.randint(-15, 15, Np)
    px = np.where(pxnew > w - 1, w - 1, px)
    py = np.where(pxnew > h - 1, h - 1, py)
    px = np.where(pxnew < 0, 0, px)
    py = np.where(pynew < 0, 0, py)
    for i in range(Np):
